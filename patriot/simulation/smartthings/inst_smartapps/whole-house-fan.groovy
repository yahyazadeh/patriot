...
def checkThings(evt) {
    ...
    def somethingOpen = settings .checkContacts == 'No' || settings .contacts?.find({ 
    ...
    if ( shouldRun && !( state .fanRunning)) {
        parent.verify(app.getLabel(), evt , fans.getDisplayName(), 'on', null) == true ? fans.on() : log.debug('Invariants Violation!')
        state .fanRunning = true
    } else {
        if (!( shouldRun ) && state .fanRunning) {
            parent.verify(app.getLabel(), evt , fans.getDisplayName(), 'off', null) == true ? fans.off() : log.debug('Invariants Violation!')
            state .fanRunning = false
        }
    }
}
