...
def doorKnock() {
    if (openSensor.latestValue('contact') == 'closed' && now() - 60 * 1000 > state .lastClosed) {
        def kSensor = knockSensor .label ? knockSensor .label : knockSensor .name
        log.debug("$kSensor detected a knock.")
        send(kSensor)
    } else {
        log.debug("$(knockSensor.label) ? knockSensor.label : knockSensor.name knocked, but looks like it was just someone opening the door.")
    }
}

def handleEvent(evt) {
    def delay = knockDelay ? knockDelay : 5
    runIn(delay, 'doorKnock')
}

private send(kSensor) {
    String code = 'SmartApps_DoorKnocker_V_0001'
    java.util.List params = [['name': 'knockSensor.label', 'value': kSensor ]]
    String msg = '{{kSensor}} detected a knock.'
    java.util.Map msgArgs = ['kSensor': kSensor ]
    java.util.Map options = ['code': code , 'params': params , 'messageArgs': msgArgs , 'translatable': true]
    java.lang.Boolean pushNotification = sendPushMessage != 'No'
    if ( pushNotification || phone ) {
        log.debug('Sending Notification')
        options += ['method': pushNotification && phone ? 'both' : pushNotification ? 'push' : 'sms', 'phone': phone ]
        parent.verify(app.getLabel(), kSensor , null, 'sending_notification', msg ) == true ? sendNotification(msg, options) : log.debug('Invariants Violation!')
    }
}
