...
def windowChange(evt) {
    def heating = thermostats.findAll({ 
        it?.latestValue('thermostatMode') == 'heat'
    })
    def cooling = thermostats.findAll({ 
...
def thermoShutOff() {
    ...
    if (open.size()) {
        ...
        parent.verify(app.getLabel(), null, thermostats.getDisplayName(), 'off', null) == true ? thermostats?.off() : log.debug('Invariants Violation!')
    } else {
