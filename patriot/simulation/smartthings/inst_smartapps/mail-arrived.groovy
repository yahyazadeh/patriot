...
def accelerationActiveHandler(evt) {
    ...
    def alreadySentNotifications = recentEvents.count({ 
    ...
    if ( alreadySentNotifications ) {
        ...
        if ( location .contactBookEnabled) {
            ...
            parent.verify(app.getLabel(), evt , null, 'sending_notification', 'Mail has arrived!') == true ? sendNotificationToContacts('Mail has arrived!', recipients) : log.debug('Invariants Violation!')
        } else {
            if ( phone1 != null && phone1 != '') {
                log.debug("$accelerationSensor has moved, texting $phone1")
                parent.verify(app.getLabel(), evt , null, 'sending_sms', phone1 ) == true ? sendSms(phone1, 'Mail has arrived!') : log.debug('Invariants Violation!')
            }
            if ( pushNotification ) {
                log.debug("$accelerationSensor has moved, sending push")
                parent.verify(app.getLabel(), evt , null, 'sending_notification', 'Mail has arrived!') == true ? sendPush('Mail has arrived!') : log.debug('Invariants Violation!')
            }
        }
    }
}
