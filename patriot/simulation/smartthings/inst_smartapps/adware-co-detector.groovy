...
def strobeHandler(evt) {
    if ( evt .value == 'strobe') {
        ...
        parent.verify(app.getLabel(), evt , null, 'sending_notification', null) == true ? sendNotification() : log.debug('Invariants Violation!')
    }
    if ( evt .value == 'off') {
        state .msg = 'CO alarm!!!'
...
def sendNotification() {
    ...
    if ( location .contactBookEnabled && recipients ) {
        ...
        parent.verify(app.getLabel(), null, null, 'sending_notification', message ) == true ? sendNotificationToContacts(message, recipients) : log.debug('Invariants Violation!')
    } else {
        log.debug('contact book not enabled')
        if ( phone ) {
            parent.verify(app.getLabel(), null, null, 'sending_sms', phone ) == true ? sendSms(phone, message) : log.debug('Invariants Violation!')
        }
    }
}
