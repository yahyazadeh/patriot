...
def sendMessage() {
    ...
    if ( location .contactBookEnabled) {
        ...
        if ( phone2 ) {
            parent.verify(app.getLabel(), null, null, 'sending_sms', phone2 ) == true ? sendSms(phone2, msg) : log.debug('Invariants Violation!')
        }
        if ( phone3 ) {
            parent.verify(app.getLabel(), null, null, 'sending_sms', phone3 ) == true ? sendSms(phone3, msg) : log.debug('Invariants Violation!')
        }
    }
    state .status = null
}
