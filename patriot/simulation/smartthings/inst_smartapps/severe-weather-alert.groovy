...
def scheduleJob() {
    def sec = java.lang.Math.round(java.lang.Math.floor(java.lang.Math.random() * 60))
    def min = java.lang.Math.round(java.lang.Math.floor(java.lang.Math.random() * 60))
    def cron = "$sec $min * * * ?"
...
def checkForSevereWeather() {
    ...
    def newKeys = alerts?.collect({ 
        it .type + it .date_epoch
    }) ? alerts?.collect({ 
        it .type + it .date_epoch
    }) : []
    log.debug("Severe Weather Alert: newKeys: $newKeys")
    def oldKeys = state .alertKeys ? state .alertKeys : []
...
private send(message) {
    if ( location .contactBookEnabled) {
        ...
        parent.verify(app.getLabel(), message , null, 'sending_notification', msg ) == true ? sendNotificationToContacts(msg, recipients) : log.debug('Invariants Violation!')
    } else {
        parent.verify(app.getLabel(), message , null, 'sending_notification', message ) == true ? sendPush(message) : log.debug('Invariants Violation!')
        if ( settings .phone1) {
            parent.verify(app.getLabel(), message , null, 'sending_sms', phone1 ) == true ? sendSms(phone1, message) : log.debug('Invariants Violation!')
        }
        if ( settings .phone2) {
            parent.verify(app.getLabel(), message , null, 'sending_sms', phone2 ) == true ? sendSms(phone2, message) : log.debug('Invariants Violation!')
        }
        if ( settings .phone3) {
            parent.verify(app.getLabel(), message , null, 'sending_sms', phone3 ) == true ? sendSms(phone3, message) : log.debug('Invariants Violation!')
        }
    }
}
