...
def accelerationInactiveHandler(evt) {
    ...
    if ( state .isRunning) {
        ...
        if (!( state .stoppedAt)) {
            state .stoppedAt = now()
            def delay = java.lang.Math.floor( fillTime * 60).toInteger()
...
def checkRunning() {
    ...
    if ( state .isRunning) {
        ...
        def sensorStates = sensor1.statesSince('acceleration', new java.util.Date(((now() - fillTimeMsec ) as java.lang.Long)))
        if (!(sensorStates.find({ 
            it .value == 'active'
        }))) {
            ...
            if ( duration - fillTimeMsec > cycleTimeMsec ) {
                ...
                if ( switches ) {
                    if (lightMode?.equals('Turn On Lights')) {
                        parent.verify(app.getLabel(), null, switches.getDisplayName(), 'on', null) == true ? switches.on() : log.debug('Invariants Violation!')
                    } else {
                        flashLights()
                    }
                }
...
private flashLights() {
    def doFlash = true
    def onFor = onFor ? onFor : 1000
    def offFor = offFor ? offFor : 1000
    def numFlashes = numFlashes ? numFlashes : 3
        ...
        numFlashes.times({ 
            ...
            switches.eachWithIndex({ s, i ->
                if ( initialActionOn [ i ]) {
                    parent.verify(app.getLabel(), null, s.getDisplayName(), 'on', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'on', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'on', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'on', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'on', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'on', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'on', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'on', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'on', ['delay': delay ]) == true ? s.on(['delay': delay ]) : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!')
                } else {
                    parent.verify(app.getLabel(), null, s.getDisplayName(), 'off', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'off', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'off', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'off', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'off', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'off', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'off', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'off', ['delay': delay ]) == true ? parent.verify(app.getLabel(), null, s.getDisplayName(), 'off', ['delay': delay ]) == true ? s.off(['delay': delay ]) : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!')
                }
            })
            delay += onFor 
