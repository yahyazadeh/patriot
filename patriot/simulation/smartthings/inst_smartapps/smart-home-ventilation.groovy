...
def turnOffSwitch() {
    switches.each({ 
        if (it.currentValue('switch') == 'on') {
            parent.verify(app.getLabel(), null, it.getDisplayName(), 'off', null) == true ? parent.verify(app.getLabel(), null, it.getDisplayName(), 'off', null) == true ? parent.verify(app.getLabel(), null, it.getDisplayName(), 'off', null) == true ? it.off() : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!')
        }
    })
...
def schedDesc(on1, off1, on2, off2, on3, off3, on4, off4, modeList, dayList) {
    ...
    if ( modeList ) {
        ...
        modeListClean = "$modeListCleanall modes"
    }
    if ( on1 && off1 ) {
...
def humanReadableTime(dateTxt) {
    new java.util.Date().parse('yyyy-MM-dd\'T\'HH:mm:ss.SSSZ', dateTxt).format('h:mm a', timeZone(dateTxt))
}

def convertEpoch(epochDate) {
    new java.util.Date(epochDate).format('yyyy-MM-dd\'T\'HH:mm:ss.SSSZ', location .timeZone)
}
