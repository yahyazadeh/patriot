...
def handleThings(evt) {
    ...
    def h = ((0.0) as java.math.BigDecimal)
    if ( settings .app_enabled) {
    ...
    if ( fan_state ) {
        if ( state .fansOn == false) {
            ...
            if ( settings .fan_control_enabled) {
                ...
                parent.verify(app.getLabel(), fan_state , fans.getDisplayName(), 'on', null) == true ? fans.on() : log.debug('Invariants Violation!')
            } else {
        ...
        if ( state .fansOn == true) {
            ...
            BigInteger ms = new BigInteger( state .fansLastRunTime)
            int seconds = (((( ms ) as BigInteger) / 1000 % 60) as BigInteger)
            int minutes = (((( ms ) as BigInteger) / 1000 * 60 % 60) as BigInteger)
            int hours = (((( ms ) as BigInteger) / 1000 * 60 * 60 % 24) as BigInteger)
            int days = (((( ms ) as BigInteger) / 1000 * 60 * 60 * 24) as BigInteger)
            def sb = String.format("$app.label cycle: %d:%02d:%02d:%02d", days, hours, minutes, seconds)
            send(sb)
            if ( settings .fan_control_enabled) {
                parent.verify(app.getLabel(), fan_state , fans.getDisplayName(), 'off', null) == true ? fans.off() : log.debug('Invariants Violation!')
                if ( emeters ) {
                    log.debug(emeters.currentValue('energy'))
                    parent.verify(app.getLabel(), fan_state , emeters.getDisplayName(), 'poll', null) == true ? emeters.poll() : log.debug('Invariants Violation!')
                    state .fansLastRunEnergy = emeters.currentValue('energy').sum()
                    state .fansLastRunCost = state .fansLastRunEnergy * price_kwh / 100.0
                    send("$app.label cycle: $state.fansLastRunEnergykWh @ $$state.fansLastRunCost")
                }
            } else {
                send("$app.label fan control is disabled.")
            }
