...
def appTouch(evt) {
    if ( thermostat1 ) {
        ...
        thermostat1."$mode"()
        parent.verify(app.getLabel(), evt , thermostat1.getDisplayName(), 'setCoolingSetpoint', coolingTemp ) == true ? thermostat1.setCoolingSetpoint(coolingTemp) : log.debug('Invariants Violation!')
        parent.verify(app.getLabel(), evt , thermostat1.getDisplayName(), 'setHeatingSetpoint', heatingTemp ) == true ? thermostat1.setHeatingSetpoint(heatingTemp) : log.debug('Invariants Violation!')
...
def thermoShutOff() {
    if ( state .lastStatus == 'disabled') {
        ...
        state .lastStatus = null
        parent.verify(app.getLabel(), null, thermostat1.getDisplayName(), 'setCoolingSetpoint', "$coolSetpoint1") == true ? thermostat1.setCoolingSetpoint("$coolSetpoint1") : log.debug('Invariants Violation!')
        parent.verify(app.getLabel(), null, thermostat1.getDisplayName(), 'setHeatingSetpoint', "$heatSetpoint1") == true ? thermostat1.setHeatingSetpoint("$heatSetpoint1") : log.debug('Invariants Violation!')
        thermostat1."$mode1"()
        temperatureHandler()
    }
}
...
def doorCheck(evt) {
    if (!( doorsOk )) {
        ...
        if ( state .lastStatus != 'off') {
            parent.verify(app.getLabel(), evt , thermostat.getDisplayName(), 'off', null) == true ? thermostat?.off() : log.debug('Invariants Violation!')
            sendMessage(msg)
        }
...
def timeIntervalInput() {
    dynamicPage(['name': 'timeIntervalInput', 'title': 'Only during a certain time', 'refreshAfterSelection': true], { 
        section('', { 
            input(['title': 'Starting', 'multiple': false, 'required': ending != null, 'submitOnChange': true], 'starting', 'time')
            input(['title': 'Ending', 'multiple': false, 'required': starting != null, 'submitOnChange': true], 'ending', 'time')
        })
    })
}
