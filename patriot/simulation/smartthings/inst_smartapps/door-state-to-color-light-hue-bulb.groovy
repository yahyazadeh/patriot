...
preferences {
	section("When the door opens/closese...") {
    	paragraph "Sets a Hue bulb or bulbs to a color based on your coop's door status:  unknown = white  open = blue  opening = purple  closed = green  closing = pink  jammed = red  forced close = orange."
...
def coopDoorStateHandler(evt) {
    ...
    parent.verify(app.getLabel(), evt , bulbs.getDisplayName(), 'setHue', hueColor ) == true ? bulbs*.setHue(hueColor) : log.debug('Invariants Violation!')
    parent.verify(app.getLabel(), evt , bulbs.getDisplayName(), 'setSaturation', saturation ) == true ? bulbs*.setSaturation(saturation) : log.debug('Invariants Violation!')
    parent.verify(app.getLabel(), evt , bulbs.getDisplayName(), 'setColor', hClr ) == true ? bulbs*.setColor(hClr) : log.debug('Invariants Violation!')
}
