...
def appTouch(evt) {
    ...
    parent.verify(app.getLabel(), evt , lock1.getDisplayName(), 'locked', null) == true ? lock1.lock() : log.debug('Invariants Violation!')
    parent.verify(app.getLabel(), evt , switcheson.getDisplayName(), 'on', null) == true ? switcheson.on() : log.debug('Invariants Violation!')
    def delay = waitfor != null && waitfor != '' ? waitfor * 1000 : 120000
    parent.verify(app.getLabel(), evt , switchesoff.getDisplayName(), 'off', ['delay': delay ]) == true ? switchesoff.off(['delay': delay ]) : log.debug('Invariants Violation!')
}
