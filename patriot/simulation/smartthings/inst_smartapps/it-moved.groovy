...
def accelerationActiveHandler(evt) {
    def deltaSeconds = 5
    def timeAgo = new java.util.Date(now() - 1000 * deltaSeconds )
    ...
    if ( alreadySentSms ) {
        ...
        if ( location .contactBookEnabled) {
            ...
            parent.verify(app.getLabel(), evt , null, 'sending_notification', "$(accelerationSensor.label) ? accelerationSensor.label : accelerationSensor.name moved") == true ? sendNotificationToContacts("$(accelerationSensor.label) ? accelerationSensor.label : accelerationSensor.name moved", recipients) : log.debug('Invariants Violation!')
        } else {
            log.debug('accelerationSensor has moved, sending text message')
            parent.verify(app.getLabel(), evt , null, 'sending_sms', phone1 ) == true ? sendSms(phone1, "$(accelerationSensor.label) ? accelerationSensor.label : accelerationSensor.name moved") : log.debug('Invariants Violation!')
        }
    }
}
