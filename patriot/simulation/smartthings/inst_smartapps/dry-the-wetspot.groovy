...
def waterHandler(evt) {
    ...
    if ( evt .value == 'wet') {
        parent.verify(app.getLabel(), evt , pump.getDisplayName(), 'on', null) == true ? pump.on() : log.debug('Invariants Violation!')
    } else {
        if ( evt .value == 'dry') {
            parent.verify(app.getLabel(), evt , pump.getDisplayName(), 'off', null) == true ? pump.off() : log.debug('Invariants Violation!')
        }
    }
}
