...
def eventHandler(evt) {
    ...
    parent.verify(app.getLabel(), evt , thermostatDevices.getDisplayName(), 'setThermostatMode', 'off') == true ? thermostatDevices*.setThermostatMode('off') : log.debug('Invariants Violation!')
    if ( frequency ) {
...
private sendMessage(evt) {
    ...
    if ( location .contactBookEnabled) {
        parent.verify(app.getLabel(), evt , null, 'sending_notification', msg ) == true ? sendNotificationToContacts(msg, recipients, options) : log.debug('Invariants Violation!')
    } else {
        if ( phone ) {
            options .phone = phone 
            if ( pushAndPhone != 'No') {
        ...
        parent.verify(app.getLabel(), evt , null, 'sending_notification', msg ) == true ? sendNotification(msg, options) : log.debug('Invariants Violation!')
    }
    if ( frequency ) {
