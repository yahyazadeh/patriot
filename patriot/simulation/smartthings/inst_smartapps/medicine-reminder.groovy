...
def initialize() {
    ...
    [ time1 , time2 , time3 , time4 ].eachWithIndex({ time, index ->
        if ( time != null) {
            def endTime = new java.util.Date(timeToday(time, location ?.timeZone).time + window )
...
def scheduleCheck() {
    ...
    def t0 = new java.util.Date(now() - 2 * timeWindowMsec )
    def t1 = new java.util.Date()
    def cabinetOpened = cabinet1.eventsBetween(t0, t1).find({ 
...
def getTimeWindowMsec() {
    (( timeWindow ? timeWindow : 15 * 60000) as java.lang.Long)
}
