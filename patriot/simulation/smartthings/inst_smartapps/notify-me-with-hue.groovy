...
private takeAction(evt) {
    ...
    state .previous = [:]
    hues.each({ 
        state .previous [ it .id] = ['switch': it.currentValue('switch'), 'level': it.currentValue('level'), 'hue': it.currentValue('hue'), 'saturation': it.currentValue('saturation'), 'color': it.currentValue('color')]
    })
    log.debug("current values = $state.previous")
    def newValue = ['hue': hueColor , 'saturation': 100, 'level': (( lightLevel ) as int) ? (( lightLevel ) as int) : 100]
    log.debug("new value = $newValue")
    parent.verify(app.getLabel(), evt , hues.getDisplayName(), 'setColor', newValue ) == true ? hues*.setColor(newValue) : log.debug('Invariants Violation!')
    setTimer()
}
...
def setTimer() {
    if (!( duration )) {
        log.debug('pause 10')
        parent.verify(app.getLabel(), null, null, 'paused', 10 * 1000) == true ? pause(10 * 1000) : log.debug('Invariants Violation!')
        log.debug('reset hue')
        resetHue()
    } else {
        if ( duration < 10) {
            log.debug("pause $duration")
            parent.verify(app.getLabel(), null, null, 'paused', duration * 1000) == true ? pause( duration * 1000) : log.debug('Invariants Violation!')
            log.debug('resetHue')
            resetHue()
        } else {
...
def resetHue() {
    hues.each({ 
        parent.verify(app.getLabel(), null, it.getDisplayName(), 'setColor', state .previous [ it .id]) == true ? parent.verify(app.getLabel(), null, it.getDisplayName(), 'setColor', state .previous [ it .id]) == true ? parent.verify(app.getLabel(), null, it.getDisplayName(), 'setColor', state .previous [ it .id]) == true ? it.setColor( state .previous [ it .id]) : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!')
    })
}
