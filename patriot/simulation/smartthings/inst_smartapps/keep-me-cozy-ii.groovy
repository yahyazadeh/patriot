...
private evaluate() {
    if ( sensor ) {
        ...
        log.trace("evaluate:, mode: $tm -- temp: $ct, heat: $thermostat.currentHeatingSetpoint, cool: $thermostat.currentCoolingSetpoint -- " + "sensor: $currentTemp, heat: $heatingSetpoint, cool: $coolingSetpoint")
        if ( tm in ['cool', 'auto']) {
            if ( currentTemp - coolingSetpoint >= threshold ) {
                parent.verify(app.getLabel(), null, thermostat.getDisplayName(), 'setCoolingSetpoint', ct - 2) == true ? thermostat.setCoolingSetpoint( ct - 2) : log.debug('Invariants Violation!')
                log.debug("thermostat.setCoolingSetpoint($(ct - 2)), ON")
            } else {
                if ( coolingSetpoint - currentTemp >= threshold && ct - thermostat .currentCoolingSetpoint >= threshold ) {
                    parent.verify(app.getLabel(), null, thermostat.getDisplayName(), 'setCoolingSetpoint', ct + 2) == true ? thermostat.setCoolingSetpoint( ct + 2) : log.debug('Invariants Violation!')
        ...
        if ( tm in ['heat', 'emergency heat', 'auto']) {
            if ( heatingSetpoint - currentTemp >= threshold ) {
                parent.verify(app.getLabel(), null, thermostat.getDisplayName(), 'setHeatingSetpoint', ct + 2) == true ? thermostat.setHeatingSetpoint( ct + 2) : log.debug('Invariants Violation!')
                log.debug("thermostat.setHeatingSetpoint($(ct + 2)), ON")
            } else {
                if ( currentTemp - heatingSetpoint >= threshold && thermostat .currentHeatingSetpoint - ct >= threshold ) {
                    parent.verify(app.getLabel(), null, thermostat.getDisplayName(), 'setHeatingSetpoint', ct - 2) == true ? thermostat.setHeatingSetpoint( ct - 2) : log.debug('Invariants Violation!')
        ...
        parent.verify(app.getLabel(), null, thermostat.getDisplayName(), 'setHeatingSetpoint', heatingSetpoint ) == true ? thermostat.setHeatingSetpoint(heatingSetpoint) : log.debug('Invariants Violation!')
        parent.verify(app.getLabel(), null, thermostat.getDisplayName(), 'setCoolingSetpoint', coolingSetpoint ) == true ? thermostat.setCoolingSetpoint(coolingSetpoint) : log.debug('Invariants Violation!')
        parent.verify(app.getLabel(), null, thermostat.getDisplayName(), 'poll', null) == true ? thermostat.poll() : log.debug('Invariants Violation!')
    }
}
