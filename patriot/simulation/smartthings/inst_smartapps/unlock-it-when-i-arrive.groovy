...
def presence(evt) {
    def anyLocked = lock1.count({ 
        it .currentLock == 'unlocked'
    }) != lock1.size()
    if ( anyLocked ) {
        parent.verify(app.getLabel(), evt , null, 'sending_notification', "Unlocked door due to arrival of $evt.displayName") == true ? sendPush("Unlocked door due to arrival of $evt.displayName") : log.debug('Invariants Violation!')
