...
def scheduleCheck() {
    ...
    def midnight = new java.util.Date().clearTime()
    ...
    def feederOpened = feederEvents.count({ 
    ...
    if ( feederOpened ) {
        ...
        if ( location .contactBookEnabled) {
            ...
            parent.verify(app.getLabel(), null, null, 'sending_notification', 'No one has fed the dog') == true ? sendNotificationToContacts('No one has fed the dog', recipients) : log.debug('Invariants Violation!')
        } else {
            log.debug("Feeder was not opened since $midnight, texting one phone number")
            parent.verify(app.getLabel(), null, null, 'sending_sms', phone1 ) == true ? sendSms(phone1, 'No one has fed the dog') : log.debug('Invariants Violation!')
        }
    }
}
