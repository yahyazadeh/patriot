...
def openValve() {
    ...
    parent.verify(app.getLabel(), null, valves1.getDisplayName(), 'on', null) == true ? valves1.on() : log.debug('Invariants Violation!')
}

def closeValve() {
    log.debug("Turning off Sprinklers $valves1")
    parent.verify(app.getLabel(), null, valves1.getDisplayName(), 'off', null) == true ? valves1.off() : log.debug('Invariants Violation!')
}

def valveOnHandler(evt) {
    log.debug("Valve $valves1 turned: $evt.value")
    def delay = duration 
    log.debug("Turning off in $(duration / 60) minutes ($delayseconds)")
