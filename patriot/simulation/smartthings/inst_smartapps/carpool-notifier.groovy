...
def riderIsHome() {
    ...
    def minusFive = new java.util.Date(['minutes': now .minutes - 5])
...
def sendText() {
    if ( location .contactBookEnabled) {
        parent.verify(app.getLabel(), null, null, 'sending_notification', message ? message : 'Your ride is here!') == true ? sendNotificationToContacts( message ? message : 'Your ride is here!', recipients) : log.debug('Invariants Violation!')
    } else {
        parent.verify(app.getLabel(), null, null, 'sending_sms', phoneNumber ) == true ? sendSms(phoneNumber, message ? message : 'Your ride is here!') : log.debug('Invariants Violation!')
    }
}
