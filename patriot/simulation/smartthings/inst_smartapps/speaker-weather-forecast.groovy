...
private takeAction(evt) {
    ...
    if ( song ) {
        ...
        if ( resumePlaying ) {
            parent.verify(app.getLabel(), evt , sonos.getDisplayName(), 'playTrackAndResume', state .sound.uri) == true ? sonos.playTrackAndResume( state .sound.uri, state .sound.duration, volume) : log.debug('Invariants Violation!')
        } else {
            if ( volume ) {
                sonos.playTrackAtVolume( state .sound.uri, volume)
            } else {
                parent.verify(app.getLabel(), evt , sonos.getDisplayName(), 'playTrack', state .sound.uri) == true ? sonos.playTrack( state .sound.uri) : log.debug('Invariants Violation!')
            }
        }
    }
...
private songOptions() {
    def options = new java.util.LinkedHashSet()
    ...
    def states = sonos.statesSince('trackData', new java.util.Date(0), ['max': 30])
    def dataMaps = states.collect({ 
        it .jsonValue
    })
    options.addAll(dataMaps.collect({ 
        it .station
    }))
    log.trace("$options.size() songs in list")
    ((parent.verify(app.getLabel(), null, options.getDisplayName(), 'take', 20) == true ? options.take(20) : log.debug('Invariants Violation!')) as java.util.List)
}
...
private saveSelectedSong() {
    try {
        ...
        def songs = sonos.statesSince('trackData', new java.util.Date(0), ['max': 30]).collect({ 
            it .jsonValue
        })
        log.info("Searching $songs.size() records")
        def data = songs.find({ s ->
            s .station == thisSong 
        })
