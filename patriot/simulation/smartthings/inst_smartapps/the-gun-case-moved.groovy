...
def accelerationActiveHandler(evt) {
    ...
    if ( alreadySentSms ) {
        ...
        if ( location .contactBookEnabled) {
            parent.verify(app.getLabel(), evt , null, 'sending_notification', 'Gun case has moved!') == true ? sendNotificationToContacts('Gun case has moved!', recipients) : log.debug('Invariants Violation!')
        } else {
            log.debug("$accelerationSensor has moved, texting phone")
            parent.verify(app.getLabel(), evt , null, 'sending_sms', phone1 ) == true ? sendSms(phone1, 'Gun case has moved!') : log.debug('Invariants Violation!')
        }
    }
}
