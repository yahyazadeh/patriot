...
def outletsOn() {
    parent.verify(app.getLabel(), null, outlets.getDisplayName(), 'on', null) == true ? outlets.on() : log.debug('Invariants Violation!')
...
def scheduledTurnOff() {
    parent.verify(app.getLabel(), null, outlets.getDisplayName(), 'off', null) == true ? outlets.off() : log.debug('Invariants Violation!')
    unschedule('scheduledTurnOff')
}
