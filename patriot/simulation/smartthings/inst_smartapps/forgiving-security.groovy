...
def triggerAlarm(evt) {
    def presenceDelay = presenceDelay ? presenceDelay : 15
...
def fireAlarm() {
    if ( state .deviceTriggers.size() > 0) {
        ...
        if ( location .mode == state .triggerMode) {
            ...
            parent.verify(app.getLabel(), null, lights.getDisplayName(), 'on', null) == true ? lights?.on() : log.debug('Invariants Violation!')
