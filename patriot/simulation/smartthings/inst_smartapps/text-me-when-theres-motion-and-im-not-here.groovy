    ...
    if (presence1.latestValue('presence') == 'not present') {
        ...
        if ( alreadySentSms ) {
            ...
            if ( location .contactBookEnabled) {
                ...
                parent.verify(app.getLabel(), evt , null, 'sending_notification', "$motion1.label $motion1.name moved while you were out") == true ? sendNotificationToContacts("$motion1.label $motion1.name moved while you were out", recipients) : log.debug('Invariants Violation!')
            } else {
                log.debug("$motion1 has moved while you were out, sending text")
                parent.verify(app.getLabel(), evt , null, 'sending_sms', phone1 ) == true ? sendSms(phone1, "$motion1.label $motion1.name moved while you were out") : log.debug('Invariants Violation!')
            }
        }
    } else {
