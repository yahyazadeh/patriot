...
def sendMessage(evt) {
    ...
    parent.verify(app.getLabel(), evt , camera.getDisplayName(), 'take', null) == true ? camera.take() : log.debug('Invariants Violation!')
    (1.. burstCount ? burstCount : 5 - 1).each({ 
        parent.verify(app.getLabel(), evt , camera.getDisplayName(), 'take', ['delay': 500 * it ]) == true ? parent.verify(app.getLabel(), evt , camera.getDisplayName(), 'take', ['delay': 500 * it ]) == true ? parent.verify(app.getLabel(), evt , camera.getDisplayName(), 'take', ['delay': 500 * it ]) == true ? camera.take(['delay': 500 * it ]) : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!')
    })
    if ( location .contactBookEnabled) {
        parent.verify(app.getLabel(), evt , null, 'sending_notification', messageText ) == true ? sendNotificationToContacts(messageText, recipients) : log.debug('Invariants Violation!')
    } else {
        parent.verify(app.getLabel(), evt , null, 'sending_notification', messageText ) == true ? sendPush(messageText) : log.debug('Invariants Violation!')
        if ( phone ) {
            parent.verify(app.getLabel(), evt , null, 'sending_sms', phone ) == true ? sendSms(phone, messageText) : log.debug('Invariants Violation!')
        }
    }
}
