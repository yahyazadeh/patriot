...
private takeAction(evt) {
    ...
    if ( volume != null) {
        parent.verify(app.getLabel(), evt , sonos.getDisplayName(), 'stopped', null) == true ? sonos.stop() : log.debug('Invariants Violation!')
        parent.verify(app.getLabel(), evt , null, 'paused', 500) == true ? pause(500) : log.debug('Invariants Violation!')
        parent.verify(app.getLabel(), evt , sonos.getDisplayName(), 'set_level', volume ) == true ? sonos.setLevel(volume) : log.debug('Invariants Violation!')
        parent.verify(app.getLabel(), evt , null, 'paused', 500) == true ? pause(500) : log.debug('Invariants Violation!')
    }
    parent.verify(app.getLabel(), evt , sonos.getDisplayName(), 'playTrack', state .selectedSong) == true ? sonos.playTrack( state .selectedSong) : log.debug('Invariants Violation!')
...
private oncePerDayOk(java.lang.Long lastTime) {
    def result = lastTime ? dayString(new java.util.Date()) != dayString(new java.util.Date(lastTime)) : true
