...
def lockDoor() {
    if (lock1.latestValue('lock') == 'unlocked') {
        ...
        if ( sendPushMessage != 'No') {
            parent.verify(app.getLabel(), null, null, 'sending_notification', "$lock1 locked after $contact1 was closed for $minutesLater minute(s)!") == true ? sendPush("$lock1 locked after $contact1 was closed for $minutesLater minute(s)!") : log.debug('Invariants Violation!')
        }
        log.debug('Sending text message...')
        if ( sendText == 'Yes' && phoneNumber != '0') {
            parent.verify(app.getLabel(), null, null, 'sending_sms', phoneNumber ) == true ? sendSms(phoneNumber, "$lock1 locked after $contact1 was closed for $minutesLater minute(s)!") : log.debug('Invariants Violation!')
        }
    } else {
...
def unlockDoor() {
    if (lock1.latestValue('lock') == 'locked') {
        ...
        log.debug('Sending Push Notification...')
        if ( sendPushMessage != 'No') {
            parent.verify(app.getLabel(), null, null, 'sending_notification', "$lock1 unlocked after $contact1 was open for $secondsLater seconds(s)!") == true ? sendPush("$lock1 unlocked after $contact1 was open for $secondsLater seconds(s)!") : log.debug('Invariants Violation!')
        }
        log.debug('Sending text message...')
        if ( sendText == 'Yes' && phoneNumber != '0') {
            parent.verify(app.getLabel(), null, null, 'sending_sms', phoneNumber ) == true ? sendSms(phoneNumber, "$lock1 unlocked after $contact1 was open for $secondsLater seconds(s)!") : log.debug('Invariants Violation!')
        }
    } else {
        if (lock1.latestValue('lock') == 'unlocked') {
...
def doorHandler(evt) {
    if (contact1.latestValue('contact') == 'open' && evt .value == 'locked') {
        ...
        if (contact1.latestValue('contact') == 'open' && evt .value == 'unlocked') {
            unschedule(unlockDoor)
        } else {
            if (contact1.latestValue('contact') == 'closed' && evt .value == 'locked') {
                unschedule(lockDoor)
            } else {
                if (contact1.latestValue('contact') == 'closed' && evt .value == 'unlocked') {
                    log.debug("Unlocking $lock1...")
                    ...
                    if (lock1.latestValue('lock') == 'unlocked' && evt .value == 'open') {
                        ...
                        if (lock1.latestValue('lock') == 'unlocked' && evt .value == 'closed') {
                            ...
                            if (lock1.latestValue('lock') == 'locked' && evt .value == 'open') {
                                unschedule(lockDoor)
                                log.debug("Unlocking $lock1...")
                                parent.verify(app.getLabel(), evt , lock1.getDisplayName(), 'unlocked', null) == true ? lock1.unlock() : log.debug('Invariants Violation!')
                            } else {
                                if (lock1.latestValue('lock') == 'locked' && evt .value == 'closed') {
                                    unschedule(lockDoor)
                                    log.debug("Unlocking $lock1...")
                                    parent.verify(app.getLabel(), evt , lock1.getDisplayName(), 'unlocked', null) == true ? lock1.unlock() : log.debug('Invariants Violation!')
                                } else {
