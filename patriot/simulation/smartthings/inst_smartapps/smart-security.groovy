...
def updated() {
    ...
    parent.verify(app.getLabel(), null, alarms.getDisplayName(), 'off', null) == true ? alarms?.off() : log.debug('Invariants Violation!')
}
...
private residentsHaveBeenQuiet() {
    ...
    for (sensor : residentMotions ) {
        def recentStates = sensor.statesSince('motion', t0)
        if (recentStates.find({ 
            it .value == 'active'
        })) {
            result = false
            break
...
private isResidentMotionSensor(evt) {
    residentMotions?.find({ 
        it .id == evt .deviceId
    }) != null
}

def appTouch(evt) {
    parent.verify(app.getLabel(), evt , alarms.getDisplayName(), 'off', null) == true ? alarms?.off() : log.debug('Invariants Violation!')
...
private startAlarmSequence() {
    if ( state .alarmActive) {
        ...
        parent.verify(app.getLabel(), null, null, 'sending_notification', 'Potential intruder detected!') == true ? sendPush('Potential intruder detected!') : log.debug('Invariants Violation!')
        if ( newMode ) {
        ...
        if (silentAlarm()) {
            ...
            parent.verify(app.getLabel(), null, alarms.getDisplayName(), 'strobe', null) == true ? alarms?.strobe() : log.debug('Invariants Violation!')
            if ( location .contactBookEnabled) {
                parent.verify(app.getLabel(), null, null, 'sending_notification', textMessage ? textMessage : 'Potential intruder detected') == true ? sendNotificationToContacts( textMessage ? textMessage : 'Potential intruder detected', recipients) : log.debug('Invariants Violation!')
            } else {
                if ( phone ) {
                    parent.verify(app.getLabel(), null, null, 'sending_sms', phone ) == true ? sendSms(phone, textMessage ? textMessage : 'Potential intruder detected') : log.debug('Invariants Violation!')
                }
            }
        } else {
        ...
        if ( lights ) {
            flashLights(java.lang.Math.min((( seconds / 2) as int), 10))
        }
    }
}
...
def soundSiren() {
    if ( state .alarmActive) {
        ...
        parent.verify(app.getLabel(), null, alarms.getDisplayName(), 'both', null) == true ? alarms?.both() : log.debug('Invariants Violation!')
        if ( lights ) {
...
private flashLights(numFlashes) {
    ...
    numFlashes.times({ 
        ...
        parent.verify(app.getLabel(), numFlashes , lights.getDisplayName(), 'on', ['delay': delay ]) == true ? parent.verify(app.getLabel(), numFlashes , lights.getDisplayName(), 'on', ['delay': delay ]) == true ? parent.verify(app.getLabel(), numFlashes , lights.getDisplayName(), 'on', ['delay': delay ]) == true ? lights?.on(['delay': delay ]) : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!')
        delay += onFor 
        log.trace("Switch off after $delay msec")
        parent.verify(app.getLabel(), numFlashes , lights.getDisplayName(), 'off', ['delay': delay ]) == true ? parent.verify(app.getLabel(), numFlashes , lights.getDisplayName(), 'off', ['delay': delay ]) == true ? parent.verify(app.getLabel(), numFlashes , lights.getDisplayName(), 'off', ['delay': delay ]) == true ? lights?.off(['delay': delay ]) : log.debug('Invariants Violation!') : log.debug('Invariants Violation!') : log.debug('Invariants Violation!')
        delay += offFor 
    })
}
