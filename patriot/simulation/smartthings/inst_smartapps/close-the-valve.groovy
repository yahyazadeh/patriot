...
def waterHandler(evt) {
    ...
    if ( evt .value == 'wet') {
        parent.verify(app.getLabel(), evt , valve.getDisplayName(), 'closed', null) == true ? valve.close() : log.debug('Invariants Violation!')
    }
    if ( frequency ) {
...
private sendMessage(evt) {
    def msg = messageText ? messageText : 'We closed the valve because moisture was detected'
    ...
    if ( phone ) {
        ...
        parent.verify(app.getLabel(), evt , null, 'sending_sms', phone ) == true ? sendSms(phone, msg) : log.debug('Invariants Violation!')
    }
    if ( frequency ) {
