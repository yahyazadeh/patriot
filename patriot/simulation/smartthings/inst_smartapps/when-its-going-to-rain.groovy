...
def scheduleCheck() {
    ...
    if (isStormy(response)) {
        def open = sensors.findAll({ 
            it?.latestValue('contact') == 'open'
        })
        if ( open ) {
            if ( location .contactBookEnabled) {
                parent.verify(app.getLabel(), null, null, 'sending_notification', "A storm is a coming and the following things are open: $open.join(, )") == true ? sendNotificationToContacts("A storm is a coming and the following things are open: $open.join(, )", recipients) : log.debug('Invariants Violation!')
            } else {
                parent.verify(app.getLabel(), null, null, 'sending_sms', phone ) == true ? sendSms(phone, "A storm is a coming and the following things are open: $open.join(, )") : log.debug('Invariants Violation!')
            }
        }
    }
}
