...
private takeAction(evt) {
    ...
    if ( song ) {
        ...
        if ( resumePlaying ) {
            ...
            parent.verify(app.getLabel(), evt , sonos.getDisplayName(), 'playTrackAndRestore', state .sound.uri) == true ? sonos.playTrackAndRestore( state .sound.uri, state .sound.duration, volume) : log.debug('Invariants Violation!')
        }
    }
...
private loadText() {
    switch ( actionType ) {
        ...
        case 'Custom Message':
            if ( message ) {
                state .sound = textToSpeech( message instanceof java.util.List ? message [ 0] : message )
            } else {
