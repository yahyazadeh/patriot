...
def motionHandler(evt) {
    ...
    if ( evt .value == 'active') {
        if (enabled()) {
            ...
            parent.verify(app.getLabel(), evt , lights.getDisplayName(), 'on', null) == true ? lights.on() : log.debug('Invariants Violation!')
            state .lastStatus = 'on'
        }
...
def illuminanceHandler(evt) {
    ...
    if ( lastStatus != 'off' && evt .integerValue > 50) {
        parent.verify(app.getLabel(), evt , lights.getDisplayName(), 'off', null) == true ? lights.off() : log.debug('Invariants Violation!')
...
def turnOffMotionAfterDelay() {
    ...
    if ( state .motionStopTime && state .lastStatus != 'off') {
        ...
        if ( elapsed >= delayMinutes ? delayMinutes : 0 * 60000 - 2000) {
            ...
            parent.verify(app.getLabel(), null, lights.getDisplayName(), 'off', null) == true ? lights.off() : log.debug('Invariants Violation!')
            state .lastStatus = 'off'
        }
    }
}
