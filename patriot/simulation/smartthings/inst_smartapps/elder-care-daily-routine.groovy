...
def scheduleCheck() {
    if (noRecentContact() && noRecentMotion()) {
        def person = person1 ? person1 : 'your elder'
        ...
        if ( location .contactBookEnabled) {
            ...
            if ( phone1 ) {
                parent.verify(app.getLabel(), null, null, 'sending_sms', phone1 ) == true ? sendSms(phone1, msg) : log.debug('Invariants Violation!')
            } else {
...
private noRecentMotion() {
    if ( motion1 ) {
        ...
        log.trace("Found $(motionEvents?.size()) ? motionEvents?.size() : 0 motion events")
        if (motionEvents.find({ 
            it .value == 'active'
        })) {
...
private noRecentContact() {
    if ( contact1 ) {
        ...
        log.trace("Found $(contactEvents?.size()) ? contactEvents?.size() : 0 door events")
        if (contactEvents.find({ 
            it .value == 'open'
        })) {
...
private getSinceTime() {
    if ( time0 ) {
        ...
        return new java.util.Date(now() - 21600000)
    }
}
