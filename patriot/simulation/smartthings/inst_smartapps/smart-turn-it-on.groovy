...
def turnOn() {
    log.debug('Start')
    def dayCheck = dayOne.contains(new java.util.Date().format('EEE'))
    def dayTwo = new java.util.Date().format('EEE')
    if ( dayCheck ) {
        def presenceTwo = presenceOne.latestValue('presence').contains('present')
        if ( presenceTwo ) {
            parent.verify(app.getLabel(), null, switchOne.getDisplayName(), 'on', null) == true ? switchOne.on() : log.debug('Invariants Violation!')
...
def turnOff() {
    parent.verify(app.getLabel(), null, switchOne.getDisplayName(), 'off', null) == true ? switchOne.off() : log.debug('Invariants Violation!')
}
