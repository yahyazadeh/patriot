...
def triggerContact(evt) {
    if (everyoneIsAway()) {
        send("Safe Watch: $(contact.label) ? contact.label : contact.name was opened!")
    }
}

def triggerMotion(evt) {
    if (everyoneIsAway()) {
        send("Safe Watch: $(motion.label) ? motion.label : motion.name sensed motion!")
    }
}

def triggerKnock(evt) {
    if (everyoneIsAway()) {
        send("Safe Watch: $(knock.label) ? knock.label : knock.name was knocked!")
    }
}

def triggerTemp(evt) {
    def temperature = evt .doubleValue
    if ( maxTemp && maxTemp < temperature || minTemp && minTemp > temperature ) {
        send("Safe Watch: $(temp.label) ? temp.label : temp.name is $temperature")
    }
}

def triggerAxis(evt) {
    if (everyoneIsAway()) {
        send("Safe Watch: $(axis.label) ? axis.label : axis.name was tilted!")
    }
}
